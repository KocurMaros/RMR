...
private:
    ...
    std::vector<std::vector<uint16_t>> hash_map; // 0 free 1 occupied
    std::vector<std::vector<Point>> coordinates;  // middle is 5 and 5
    int minX = 100000, minY = 100000, maxX = -1000000, maxY = -1000000;
    int boarder_minX = 100000, boarder_minY = 100000, boarder_maxX = -1000000, boarder_maxY = -1000000;

public:
    Hash_map() = default;
    Hash_map(double x, double y, double theta, uint8_t square_dim, uint8_t map_dim);
    ~Hash_map();
    void update_map(Point point, uint16_t occupied);
    std::vector<std::vector<uint16_t>> get_hash_map();
